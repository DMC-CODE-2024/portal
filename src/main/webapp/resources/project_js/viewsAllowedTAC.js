...
function allowedDatatable(){
	...
	$.ajax({
		url: 'headers?type=allowedTACHeaders',
		type: 'POST',
		dataType: "json",
		...
		success: function(data){
			...
			var date=data.inputTypeDateList;
				...
				for(i=0; i<date.length; i++){
					if(date[i].type === "date"){  
								$("#allowedTACTableDiv").append("<div class='form-group'>"+
						...
						$("#allowedTACTableDiv").append("<div class='form-group'><label for="+date[i].id+" class='center-align'>"+date[i].title+"</label><input type="+date[i].type+" class='form-control' id="+date[i].id+" placeholder='Enter here' maxlength='19'/></div>");
						
					}
					else if(date[i].type === "select"){
					var dropdownDiv=
							$("#allowedTACTableDiv").append(
			...
			/*$('#allowedTACTableDiv div:last').after("<p id='errorMsg' style='padding:5px;color: red;font-size: 15px;position: absolute;left: 23px;margin: 0;top: 57px;' class='left'></p>")	*/
			$('#errorMsgDiv').append("<p id='errorMsg' style='color: red;font-size: 15px;left: 23px;margin: 0;' class='left'></p>")	  	  
			
			$("#allowedTACTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-light' id='clearFilter'>"+$.i18n('clearFilter')+"</button></div>");
			$("#allowedTACTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-primary' id='submitFilter'><span><img src='./resources/assets/images/filter-icon.svg' alt='icon' class='img-fluid ml-1'></span></button></div>");
			$("#allowedTACTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-outline-dark' onclick='exportData()'>"+$.i18n('Export')+"<img src='./resources/assets/images/download-icon.svg' alt='icon' class='img-fluid ml-1'></button>");
