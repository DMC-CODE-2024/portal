...
return x;
	...
	var {startDate,endDate,tac,txnId,category,addedBy,uploadedBy} = {
        tac : isEmpty($('#tac').val()),
...
function table(){
	...
	$.ajaxSetup({
		...
		{ 'X-CSRF-TOKEN': token }
				...
				$.ajax({
    				url: './headers?type=block-tac-list-mgmt',
    				type: 'POST',
    				dataType: "json",
			...
			var elem='<h1>'+data.pageTitle+'<h1>';
			 $("#pageHeader").append(elem);
		      	  $("#pageHeader").append('<div class="toggle-row ml-auto mr-4"></div>');
                                         $("#pageHeader").append('<a href="./blocked-tac-list?FeatureId='+featureId+'" target="mainArea"><button type="button"  id="btnLink"  class="btn btn-outline-dark">Back</button></a>');
...
function filterDropdown(){
                        				...
                        				$.getJSON('./getDropdownList/BLOCK_TAC_CATEGORY_TYPE', function(data) {
