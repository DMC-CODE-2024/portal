...
function configManagementDatatable(){
	...
	var filterRequest={
			...
			"language":$('#language').val() == '' ? null : $('#language').val(),
	...
	$.ajax({
		url: 'headers?type=checkIMEIConfigMessage',
		type: 'POST',
		dataType: "json",
   ...
   .off().on('keyup', function(event) {
	   if (event.keyCode === 13) {
			 table.search(this.value.trim(), false, false).draw();
				...
				"columns": result,
				fixedColumns: true,
				columnDefs: [{targets:[3,4,5], class:"columnwrap"}]
			});
...
return new Promise((resolve, reject) => {
	...
	$.ajax({
		...
		success: function(data){
			...
			/*$("#configTableButtonDiv").append("<div class='form-group'><input type='number' id='gotoPageId' class='gotoPages' style='width: 50px;height: 37px;text-align: center;'>&nbsp;<button type='button' class='btn btn-outline-dark' id='submitPagination' onclick='gotoPagination()'>"+$.i18n('Go To')+"</button>");*/
			$("#configTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-light' id='clearFilter'>Reset All</button></div>");
			$("#configTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-primary' id='submitFilter' onclick='configManagementDatatable()'>Apply Filter <img src='./resources/assets/images/filter-icon.svg' alt='icon' class='img-fluid ml-1'></button></div>");
			$("#configTableButtonDiv").append("<div class='form-group'><button type='button' class='btn btn-outline-dark' onclick='exportData()'>Export <img src='./resources/assets/images/download-icon.svg' alt='icon' class='img-fluid ml-1'></button>");
		...
		/*	for(i=0; i<button.length; i++){
			...
			/*$('#submitPagination').attr("onclick", "submitPagination('viewFilter')");
			for(i=0; i<button.length; i++){
				$('#'+button[i].id).text(button[i].buttonTitle);
				$('#'+button[i].id).attr("onclick", button[i].buttonURL);
...
/*	$.getJSON('./getDistinctTypeName', function(data) {
	...
	$.getJSON('./getDropdownList/LANGUAGE_IMEI_MGMT', function(data) {
    				for (i = 0; i < data.length; i++) {
	...
	$.getJSON('./getDistinctIMEIFeatureName', function(data) {
		  for (i = 0; i < data.length; i++) {
...
function setViewPopupData(data){
	...
	data.language=="" || data.language==null ? $("#viewLanguage").val('NA') : $("#viewLanguage").val(data.language);
	data.value=="" || data.value==null ? $("#viewValue").val('NA') : $("#viewValue").val(data.value);
	data.typeInterp=="" || data.type==null ? $("#viewtype").val('NA') : $("#viewtype").val(data.type);
...
function setEditPopupData(data){
	data.tag=="" || data.tag==null ? $("#editTag").val('NA') : $("#editTag").val(data.tag);
	$("#EditId").val(data.id);
		...
		/*	$("#editAdminSystemModel").closeModal();
			...
			$.i18n().load({
                ...
                $('#updateFieldMessage').text(data.message);*/
                     ...
                     window.parent.alertify.success(data.message,4);
			});
...
function submitPagination(formID){
