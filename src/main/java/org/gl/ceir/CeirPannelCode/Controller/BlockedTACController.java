...
private final Logger log = LoggerFactory.getLogger(getClass());
	...
	@RequestMapping(value=
		{"/blockedTAC"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view BlockedTAC entry point."); 
		 mv.setViewName("viewBlockedTAC");
		log.info(" view BlockedTAC exit point."); 
		return mv; 
	}
	
	
	@PostMapping("/blockedTAC/viewTag") 
	public @ResponseBody ConfigContentModel VipListViewTag (@RequestBody FilterRequest filterRequest,HttpSession session)  {
		...
		ConfigContentModel response= feignCleintImplementation.viewBlockedTacFeign(filterRequest);
		log.info("viplist response from currency api "+response);
		return response;
	}
	
	
	@PutMapping("/blockedTAC/update")
	...
	@PostMapping("exportBlockedTACData")
	@ResponseBody
	public FileExportResponse exportToExcel(@RequestBody FilterRequest filterRequest,HttpSession session)
	{
		...
		response= feignCleintImplementation.blockedTacView(filterRequest, filterRequest.getPageNo(), filterRequest.getPageSize(), file);
