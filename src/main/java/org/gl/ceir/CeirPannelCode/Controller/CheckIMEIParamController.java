...
public class CheckIMEIParamController {
	...
	@RequestMapping(value=
		{"/CheckIMEIParam"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view viewCheckIMEIManagement entry point."); 
		 mv.setViewName("viewCheckIMEIManagement");
		log.info(" view viewCheckIMEIManagement exit point."); 
		return mv; 
	}
	
	
	@PostMapping("/checkimei/viewTag") 
	public @ResponseBody ConfigContentModel SystemConfigViewTag (@RequestBody FilterRequest filterRequest,HttpSession session)  {
		...
		ConfigContentModel response= feignCleintImplementation.checkIMEIParam(filterRequest);
		log.info("response from currency api "+response);
		return response;
	}
	
	
	@PutMapping("/checkimei/update")
	public @ResponseBody GenricResponse updateSystem (@RequestBody ConfigContentModel configContentModel,HttpSession session) {
		...
		GenricResponse Response = feignCleintImplementation.checkIMEIParam(configContentModel);
	...
	@PostMapping("exportCheckIMEIData")
	@ResponseBody
	public FileExportResponse exportToExcel(@RequestBody FilterRequest filterRequest,HttpSession session)
	{
		...
		response= feignCleintImplementation.checkIMEIparam(filterRequest, filterRequest.getPageNo(), filterRequest.getPageSize(), file);
