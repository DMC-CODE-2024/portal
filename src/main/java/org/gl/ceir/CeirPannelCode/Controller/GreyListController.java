...
public class GreyListController {
	...
	@RequestMapping(value=
		{"/greyListView"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view Grey List ConfigManagement entry point."); 
		 mv.setViewName("viewGreyListManagement");
		log.info(" view Grey Lis Management exit point."); 
		return mv; 
	}
	
	
	@PostMapping("/greylist/viewTag") 
	public @ResponseBody ConfigContentModel greyListViewTag (@RequestBody FilterRequest filterRequest,HttpSession session)  {
		...
		ConfigContentModel response= feignCleintImplementation.viewGreylistFeign(filterRequest);
		log.info("viplist response from currency api "+response);
		return response;
	}
	
	
	@PutMapping("/greylist/update")
	public @ResponseBody GenricResponse updateGreyList (@RequestBody ConfigContentModel configContentModel,HttpSession session) {
	...
	@PostMapping("exportGreyListData")
	@ResponseBody
	public FileExportResponse exportToExcel(@RequestBody FilterRequest filterRequest,HttpSession session)
	{
		...
		response= feignCleintImplementation.greyListView(filterRequest, filterRequest.getPageNo(), filterRequest.getPageSize(), file);
