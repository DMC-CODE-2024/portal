...
@Controller
public class CheckIMEIContentController {
	...
	@RequestMapping(value=
		{"/checkIMEIContent"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view checkIMEIContent entry point."); 
		 mv.setViewName("viewCheckIMEIContent");
		log.info(" view checkIMEIContent exit point."); 
		return mv; 
	}
	
	
	@PostMapping("/imeiContent/viewTag") 
	public @ResponseBody ConfigContentModel SystemConfigViewTag (@RequestBody FilterRequest filterRequest,HttpSession session)  {
		...
		ConfigContentModel response= feignCleintImplementation.checkIMEIContent(filterRequest);
		log.info("response from currency api "+response);
		return response;
	}
	
	
	@PutMapping("/imeiContent/update")
	public @ResponseBody GenricResponse updateSystem (@RequestBody ConfigContentModel configContentModel,HttpSession session) {
		...
		GenricResponse Response = feignCleintImplementation.checkIMEIContent(configContentModel);
	...
	@PostMapping("exportCheckIMEIContentData")
	@ResponseBody
	public FileExportResponse exportToExcel(@RequestBody FilterRequest filterRequest,HttpSession session)
	{
		...
		response= feignCleintImplementation.checkIMEIContent(filterRequest, filterRequest.getPageNo(), filterRequest.getPageSize(), file);
