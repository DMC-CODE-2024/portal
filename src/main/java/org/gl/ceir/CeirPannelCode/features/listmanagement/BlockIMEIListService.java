...
@Component
public class BlockIMEIListService {
    ...
    @Autowired
    public BlockIMEIListService(APIService6Feign apiService6Feign, DatatableResponseModel datatableResponseModel, PageElement pageElement, Button button, IconsState iconState) {
    ...
    public ResponseEntity<?> paging(HttpServletRequest request, HttpSession session) {
        ...
        BlockListEntity blockListEntity = gsonObject.fromJson(filter, BlockListEntity.class);
        logger.info("ExceptionListEntity payload [" + blockListEntity + "]");
        ...
        blockListEntity.getAuditTrailModel().setColumnName(columnName);
        blockListEntity.getAuditTrailModel().setSort(order);
        ...
        logger.info("payload pass to blockListEntity [" + blockListEntity + "]");
        ...
        Object response = apiService6Feign.blockListPaging(blockListEntity, pageNo, pageSize);
        ...
        ContentResponse tacPaging = gson.fromJson(gsonJson, new TypeToken<ContentResponse<BlockListEntity>>() {
        }.getType());

        logger.info("pagingResponse [" + tacPaging + "]");

        List<BlockListEntity> paginationContentList = tacPaging.getContent();
        ...
        if (paginationContentList.isEmpty()) {
            ...
            for (BlockListEntity dataInsideList : paginationContentList) {
    ...
    public ResponseEntity<?> pageRendering(@RequestParam(name = "type", defaultValue = "", required = false) String role, HttpSession session) {
        ...
        pageElement.setPageTitle(translator.toLocale("View Blocked IMEI List"));
    ...
    public FileExportResponse export(BlockListEntity blockListEntity, HttpSession session) {
        //  Gson gsonObject = new Gson();
        Object response;
        String userType = (String) session.getAttribute("usertype");
        ...
        response = apiService6Feign.blockListExport(blockListEntity);
