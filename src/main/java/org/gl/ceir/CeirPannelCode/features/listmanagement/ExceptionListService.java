...
public class ExceptionListService {
    ...
    @Autowired
    public ExceptionListService(APIService6Feign apiService6Feign, DatatableResponseModel datatableResponseModel, PageElement pageElement, Button button, IconsState iconState, ApproveDeviceTACFeignClient approveDeviceTACFeignClient) {
    ...
    public ResponseEntity<?> paging(HttpServletRequest request, HttpSession session) {
        ...
        ExceptionListEntity exceptionListEntity = gsonObject.fromJson(filter, ExceptionListEntity.class);
        logger.info("ExceptionListEntity payload [" + exceptionListEntity + "]");
        ...
        exceptionListEntity.getAuditTrailModel().setColumnName(columnName);
        exceptionListEntity.getAuditTrailModel().setSort(order);
        exceptionListEntity.getAuditTrailModel().setPublicIp(session.getAttribute("publicIP").toString());
        exceptionListEntity.getAuditTrailModel().setBrowser(session.getAttribute("browser").toString());
        logger.info("payload pass to exceptionListEntity [" + exceptionListEntity + "]");
        ...
        Object response = apiService6Feign.exceptionListPaging(exceptionListEntity, pageNo, pageSize);
        ...
        ContentResponse tacPaging = gson.fromJson(gsonJson, new TypeToken<ContentResponse<ExceptionListEntity>>() {
        }.getType());

        logger.info("pagingResponse [" + tacPaging + "]");

        List<ExceptionListEntity> paginationContentList = tacPaging.getContent();
        ...
        if (paginationContentList.isEmpty()) {
            ...
            for (ExceptionListEntity dataInsideList : paginationContentList) {
    ...
    private static List<Object> getFinalDataList(ExceptionListEntity dataInsideList) {
    ...
    public ResponseEntity<?> pageRendering(@RequestParam(name = "type", defaultValue = "", required = false) String role, HttpSession session) {
        ...
        pageElement.setPageTitle(translator.toLocale("View Exception IMEI List"));
    ...
    public FileExportResponse export(ExceptionListEntity exceptionListEntity, HttpSession session) {
        //  Gson gsonObject = new Gson();
        Object response;
        String userType = (String) session.getAttribute("usertype");
        int usertypeId = (int) session.getAttribute("usertypeId");
        exceptionListEntity.getAuditTrailModel().setUserType(userType);
        exceptionListEntity.getAuditTrailModel().setUserTypeId(Long.valueOf(usertypeId));
        exceptionListEntity.getAuditTrailModel().setPublicIp(session.getAttribute("publicIP").toString());
        exceptionListEntity.getAuditTrailModel().setBrowser(session.getAttribute("browser").toString());
        logger.info("filterRequest:::::::::" + exceptionListEntity);
        response = apiService6Feign.exceptionListExport(exceptionListEntity);
